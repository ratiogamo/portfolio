    ...
    <section id="portfolio" className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold font-inter mb-4">Featured Projects & Case Studies</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {projectCategories.map((category) => (
            <Link key={category.name} href={category.link}>
              <a className="block bg-white rounded-lg shadow-md transition-transform hover:scale-105 p-6 text-center">
                <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4">
                  <i className={`${category.icon} text-2xl text-primary`}></i>
                </div>
                <h3 className="text-xl font-bold font-inter mb-2">{category.name}</h3>
